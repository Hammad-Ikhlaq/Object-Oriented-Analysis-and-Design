...
public class LogIn extends javax.swing.JFrame {
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        if(BorrowerRadioButton.isSelected())
        {
            ...
            {
                int Bid=Library.getInstance().borrowers.get(i).id;
                String temp=NameTextField.getText();
                int id=Integer.valueOf(temp);
                if(Bid==id)
                {
                        String p=PasswordField.getText();
                        if(Library.getInstance().borrowers.get(i).password.equals(p))
                        {
                                java.awt.EventQueue.invokeLater(new Runnable() {
                                public void run() {
                                new BorrowerGUI(Bid).setVisible(true);
                            }
                            });
        ...
        {
            for(int i=0;i<Library.getInstance().clerks.size();i++)
            {
                int Bid=Library.getInstance().clerks.get(i).id;
                String temp=NameTextField.getText();
                int id=Integer.valueOf(temp);
                if(Bid==id)
                {
                        String p=PasswordField.getText();
                        if(Library.getInstance().clerks.get(i).password.equals(p))
                        {
                                java.awt.EventQueue.invokeLater(new Runnable() {
                                public void run() {
                                new ClerkGUI(Bid).setVisible(true);
                            }
                            });
        ...
        {
            for(int i=0;i<Library.getInstance().librarians.size();i++)
            {
                int Bid=Library.getInstance().librarians.get(i).id;
                String temp=NameTextField.getText();
                int id=Integer.valueOf(temp);
                if(Bid==id)
                {
                        String p=PasswordField.getText();
                        if(Library.getInstance().librarians.get(i).password.equals(p))
                        {
                                java.awt.EventQueue.invokeLater(new Runnable() {
                                public void run() {
                                new LibrarianGUI(Bid).setVisible(true);
                            }
                            });
